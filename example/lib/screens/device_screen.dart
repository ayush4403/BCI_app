...
class _DeviceScreenState extends State<DeviceScreen> {
  ...
  void initState() {
    ...
    _headsetStateSubscription = headset.onStateChange().listen((state) {
      ...
      if (mounted) {
        setState(() {});
        Stream<int> data = headset.onAttentionUpdate();
  ...
  Widget myapp(BuildContext context, String title, Stream stream) {
    return StreamBuilder(
        stream: stream,
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            addData(DateTime.now(), snapshot.data);
          }
          return Column(
            children: [
              Text(title, style: Theme.of(context).textTheme.bodyLarge),
    ...
    return Column(
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceEvenly,
          children: [
            myapp(context, "Attention", headset.onAttentionUpdate()),
  ...
  Widget buildRawChartWidget(BuildContext context) {
    return StreamBuilder(
        ...
        builder: (context, snapshot) {
        if (snapshot.hasData) {
            final Int32List data = snapshot.data as Int32List; // Cast to Int32List
            final int bytesPerSecond = data.length * 4; // Each Int32 is 4 bytes
            print("Bytes per second: $bytesPerSecond");

            // Print the datatype and length
            print("Print datatype: ${data.runtimeType}, Length: ${data.length}");
          }
